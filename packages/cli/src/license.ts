...
export class License implements LicenseProvider {
	...
	// CUSTOM LICENSE BYPASS: Always return true for license checks
	...
	// Support for new insights features with bypass
	...
	getInfo(): string {
		const currentEntitlements = this.getCurrentEntitlements();

		if (currentEntitlements.length === 0) {
			return 'Community';
		}

		const entitlement = this.getMainPlan();

		if (!entitlement) {
			return `Licensed (${currentEntitlements.length} entitlement${
				currentEntitlements.length === 1 ? '' : 's'
			})`;
		}

		const validFrom = entitlement.validFrom.toISOString().slice(0, 10);
		const validTo = entitlement.validTo.toISOString().slice(0, 10);

		return `${entitlement.productMetadata.name} (${validFrom} - ${validTo})`;
	}
	...
	enableAutoRenewals() {
		this.globalConfig.license.autoRenewalEnabled = true;
	}
	...
	private onExpirySoon() {
		this.logger.warn('License is expiring soon but auto-renewals are disabled');
	}
}
